...
export default function HomePage() {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 grid-cols-1">
        <Chart
          ...
          title="SLA Achievement Trend (Weekly)"
        />
        <Chart
  ...
  const domainTab1 = (
    <div className="space-y-6">
      ...
      <Chart
        ...
        type="area"
        xKey="x"
        yKeys={[{ key: 'y', name: 'Count' }]}
        title="Breach Root Cause Analysis"
        height={400}
      />
    </div>
  );
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Chart
          ...
          title="Projected Penalties (30-day)"
        />
        <ActionMemo
          persona={{ name: 'Jerome Santos', role: 'SLA Governance Manager' }}
          context={{}}
          onGenerate={async () => {
            const memos = [
              {
                ...
                actions: ['Remediate TechCorp SLA breach (response time exceeded)', 'Deploy additional resources for FinServ overnight queue', 'Negotiate penalty waiver for force majeure typhoon impact'],
              },
              {
  ...
  const askAiTab = (
    <div className="h-[600px]">
      <AskAI
        title="Ask AI"
        sampleQuestions={[
          'Which SLAs are at risk of breach this week?',
          'Show penalty cost trend by client',
          'What is the root cause of repeated email SLA breaches?',
        ]}
        mode="sql"
  ...
  const tabs = [
    ...
    { id: 'domain-1', label: 'Breach Analysis', icon: '📈', content: domainTab1 },
    { id: 'domain-2', label: 'Penalty Forecast', icon: '⚡', content: domainTab2 },
